...
<div class="modal fade" id="promote2" tabindex="-1" role="dialog"  aria-hidden="true" dir="rtl">
	<div class="modal-dialog modal-lg" role="document">
		<div class="modal-content text-right">
			<div class="modal-header">
				<h6 class="modal-title"> <strong>تثبيت الاعلان</strong> </h6>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
				</button>
			</div>
            <form action="/listings/promote" method="POST" enctype="multipart/form-data" id="promote-form2" class="ajax swal-msg" data-on-success="remove_promote_button2">
            	@csrf()
            	<input type="hidden" name="listing_id" value="">
				...
				<div class="modal-body" dir="rtl">
					...
					<div class="alert alert-warning">
						{!! Auth::user()->payout_balance() ? 'رصيدك المتاح حاليا <strong><span class="current-balance">'.Auth::user()->payout_balance()."</span> ". currency()->symbol ."</strong>, هل أنت بحاجة للمزيد لتثبيت اعلانك" : "ليس لديك رصيد بالمحفظة" !!}
						<a href="/balance" class="float-left btn btn-warning btn-sm">
							<i class="fa fa-bolt mr-1 ml-2" style="opacity: .6;"></i> قم بشحن رصيدك الآن 
							<i class="fa fa-bolt ml-1 mr-2" style="opacity: .6;"></i>
						</a>
				...
				<div class="modal-footer">
                    <button type="button" class="btn btn-default ml-1" data-dismiss="modal"> <small>تراجع</small> </button>
					<button type="button" class="btn bgPrimary text-white px-4 promote-btn2"> تثبيت الإعلان </button>
				</div>
            </form>
		</div>
	</div>
