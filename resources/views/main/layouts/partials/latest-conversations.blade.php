@forelse (Auth::user()->unique_conversations()->latest()->orderBy('updated_at', 'desc')->limit(25)->get() as $conversation)
	<a href="#" class="chat-message clearfix toggle-chat {{ $conversation->messages()->whereNull('seen')->first() ? 'unseen' : '' }}" data-name="{{ $conversation->other_partey()->name }}" data-logo="{{ $conversation->other_partey()->profile_picture() }}" data-username="{{ $conversation->other_partey()->username }}">
